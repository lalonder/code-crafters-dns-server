...
pub fn parse_question(input: &[u8]) -> IResult<&[u8], Question> {
    let (input, mut question) = match tuple((until_null, take(1usize), be_u16, be_u16))(input) {
...
fn take_name(input: &[u8]) -> IResult<&[u8], &[u8]> {
    let (input, name) = length_data(be_u8)(input)?;
    Ok((input, name))
}
