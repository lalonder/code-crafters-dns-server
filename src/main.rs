...
use bytes::BytesMut;
use crate::parser::{parse_message, parse_questions};
...
fn into_message(input: &[u8]) -> Message {
    match parse_message(input) {
        Ok((_, (header, question))) =>
            Message { header, question },
        _ => panic!(),
    }
}

fn into_response(message: &Message) -> Response {
...
fn main() {
    ...
    loop {
        match udp_socket.recv_from(&mut buf[..]) {
            Ok((_size, source)) => {
                println!("{:?}", parse_questions(&buf));
                let message = into_message(&buf);
                let response = into_response(&message);
                // println!("{:?}", response);
                udp_socket
