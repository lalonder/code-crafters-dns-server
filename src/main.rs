...
use bytes::BytesMut;
use crate::parser::{parse_header, parse_question};
...
fn create_message(input: &[u8]) -> Message {
    Message {
        header: parse_header(input),
        question: parse_question(input),
    }
}
...
fn create_response(message: &Message) -> Response {
    ...
    Response {
        ...
        answer: Answer {
            ...
            ttl: 255u32,
            rdlenth: 4u16,
            rdata: Vec::from("\x08\x08\x08\x08\x08\x08\x04\x04".as_bytes()),
        },
    }
}
...
fn main() {
    ...
    loop {
        match udp_socket.recv_from(&mut buf[..]) {
            Ok((_size, source)) => {
                let message = create_message(&buf);
